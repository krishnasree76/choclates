...
const ProductCard = ({ product, index }: ProductCardProps) => {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ duration: 0.5, delay: index * 0.1 }}
      // ✅ FIX: Higher z-index when dropdown is active ensures it stays above cards below it
      className={`glass-card group relative ${
        isFlavorOpen || isVariantOpen ? "z-50" : "z-10"
      }`}
      ...
      <div className="p-6">
        ...
        {variants.length > 0 && (
          <div className="relative mb-4">
            ...
            >
              <span className="truncate">
                {selectedVariant ? `${selectedVariant.label} - ₹${selectedVariant.price}` : "Select Variant"}
              </span>
            ...
            <AnimatePresence>
              {isVariantOpen && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  className="absolute z-[100] top-full left-0 right-0 mt-2 bg-chocolate-medium border border-primary/30 rounded-xl shadow-2xl overflow-hidden"
                >
                  <div className="max-h-48 overflow-y-auto scrollbar-gold">
                    {variants.map((v) => (
                      <button
                        key={v.label}
                        onClick={() => {
                        ...
                        className={`w-full flex items-center justify-between px-4 py-3 text-sm text-left hover:bg-primary/10 transition-colors ${
                          selectedVariantLabel === v.label ? "bg-primary/20 text-primary" : "text-cream"
                        }`}
                      >
                        <span className="truncate pr-2">{v.label} - ₹{v.price}</span>
                        {selectedVariantLabel === v.label && <Check className="w-4 h-4 text-primary" />}
                      </button>
                    ))}
                  </div>
                </motion.div>
            ...
            <AnimatePresence>
              {isFlavorOpen && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  // ✅ FIX: Added shadow and z-index
                  className="absolute z-[100] top-full left-0 right-0 mt-2 bg-chocolate-medium border border-primary/30 rounded-xl shadow-2xl overflow-hidden"
                >
                  {/* ✅ FIX: Added max-height and scrolling to prevent clipping on mobile */}
                  <div className="max-h-60 overflow-y-auto scrollbar-gold">
                    {flavors.map((flavor) => (
                      <button
                        key={flavor}
                        onClick={() => {
                        ...
                        className={`w-full flex items-center justify-between px-4 py-3 text-sm text-left hover:bg-primary/10 transition-colors ${
                          selectedFlavor === flavor ? "bg-primary/20 text-primary" : "text-cream"
                        }`}
                      >
                        <span className="truncate pr-2">{flavor}</span>
                        {selectedFlavor === flavor && <Check className="w-4 h-4 text-primary flex-shrink-0" />}
                      </button>
                    ))}
                  </div>
                </motion.div>
        ...
        <div className="text-primary font-semibold text-lg mb-4">{priceLabelToUse}</div>

        <div className="flex items-center justify-between mb-4">
          <span className="text-cream-muted text-sm">Quantity</span>
          <div className="flex items-center gap-3">
            <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-8 h-8 rounded-full border border-primary/30 flex items-center justify-center text-primary hover:bg-primary/10">-</button>
            <span className="text-cream font-semibold w-8 text-center">{quantity}</span>
            <button onClick={() => setQuantity(quantity + 1)} className="w-8 h-8 rounded-full border border-primary/30 flex items-center justify-center text-primary hover:bg-primary/10">+</button>
          </div>
        </div>

        <div className="flex gap-3">
          <button onClick={handleAddToCart} className="flex-1 bg-primary text-primary-foreground py-3 rounded-xl font-semibold flex items-center justify-center gap-2 hover:opacity-90 transition-opacity">
            <ShoppingBag className="w-4 h-4" /> Add to Cart
          </button>
          <button onClick={handleWhatsAppOrder} className="w-12 h-12 rounded-xl border border-green-500/50 flex items-center justify-center text-green-500 hover:bg-green-500/10 transition-colors">
            <MessageCircle className="w-5 h-5" />
          </button>
        </div>
      </div>
